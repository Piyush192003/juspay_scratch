...
const ThinkMessageWithTimer = ({ componentId }) => {
  const states = useSelector((state) => state.spriteUseCase);
  ...
  const displayThinkMessageWithTimer = () => {
    const el = document.getElementById(`${states.activeSprite}-message-box`);
    const el2 = document.getElementById(`${states.activeSprite}-think-box1`);
    if (state.show_msg && state.character_id === states.activeSprite) {
    ...
    <Paper elevation={3}>
      <div className='rounded text-center bg-purple-700 p-2 my-3'>
        ...
        <div
          ...
          onClick={displayThinkMessageWithTimer}
        >
...
export default ThinkMessageWithTimer;
